...
class Window(QtGui.QMainWindow):
    ...
    def Add_To_Liste(self,commandconfig): 
        row = self.ListCommand.rowCount()
        item1 = QtGui.QTableWidgetItem(str(commandconfig[0]))
        item2 = QtGui.QTableWidgetItem(str(commandconfig[1]))  
    ...
    def run(self):
        try:
            for index in range(self.ListCommand.rowCount()):
                cfg = str(self.ListCommand.item(index,1).text())
                scan = str(self.ListCommand.item(index,0).text())
                BINoculars.main.Main.from_object(cfg, scan)
    ...
    def ShowFile(self):
        ...
        #confwidget = Conf_Tab(self) 
        #QtCore.QObject.connect(confwidget, QtCore.SIGNAL("command"),self.Add_To_Liste)
        #self.tab_widget.addTab(confwidget,NameFile[0])
        #widget = self.tab_widget.currentWidget()
        #widget.read_data(filename)
